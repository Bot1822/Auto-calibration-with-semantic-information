...
std::string image_filename = "1403621972966380.png";
std::string pointcloud_filename = "1403621972909235.bin";
YAML::Node config;
bool debug = false;

//VLP-16
//int rings = 16;
//float _lowerBound = -15.0f;      //< the vertical angle of the first scan ring
//float _upperBound = 15.0;      //< the vertical angle of the last scan ring
//float _factor;

//VLP-64 -24.9f, 2, 64
int rings = 64;
float _lowerBound = -24.9f;      //< the vertical angle of the first scan ring
float _upperBound = 2.0;      //< the vertical angle of the last scan ring
float _factor;
...
Eigen::Matrix3f T_cam2image = Eigen::Matrix3f::Zero();//cam2image
                            ...
                            cv::Mat& R, cv::Mat& t )
{
    ...
    std::cout<<"ORB -- R is "<<std::endl<<R<<std::endl;
    std::cout<<"ORB -- t is "<<std::endl<<t<<std::endl;
    ...
    std::cout << "ORB -- euler_ang_zyx is " << std::endl << euler_ang_zyx * 180.0 / M_PI << std::endl;
}
...
void extract_pc_feature_5(pcl::PointCloud<pcl::PointXYZI>::Ptr pc, pcl::PointCloud<pcl::PointXYZI>::Ptr pc_feature) {
    float factor_t = ((_upperBound - _lowerBound) / (rings - 1));
    _factor = ((rings - 1) / (_upperBound - _lowerBound));
    ...
    for(int i=0; i<pc_image.size(); i++){
        ...
        for(int j=0; j<pc_image[i].size(); j++)pc_image[i][j] = -1;
    }
    for(size_t i=0; i<pc->size(); i++){
        float theta = 0;
        if(pc->points[i].y == 0)theta = 90.0;
        ...
        int col = cvFloor(theta / 0.18);
        if(col < 0 || col > 999)continue;
        float xiebian = std::sqrt(std::pow(pc->points[i].x, 2) + std::pow(pc->points[i].y, 2));
        float angle = std::atan(pc->points[i].z / xiebian);
        int ring_id = int(((angle * 180 / M_PI) - _lowerBound) * _factor + 0.5);
        if(ring_id < 0 || ring_id > rings-1)continue;
        if(pc_image[col][ring_id] == -1) {
            pc_image[col][ring_id] = std::sqrt(std::pow(pc->points[i].y, 2) + std::pow(pc->points[i].x, 2) + std::pow(pc->points[i].z, 2));
        }
        else if(std::sqrt(std::pow(pc->points[i].y, 2) + std::pow(pc->points[i].x, 2) + std::pow(pc->points[i].z, 2)) < pc_image[col][ring_id]) {
            pc_image[col][ring_id] = std::sqrt(std::pow(pc->points[i].y, 2) + std::pow(pc->points[i].x, 2) + std::pow(pc->points[i].z, 2));
        }
    }
    for(int i=0; i<pc_image.size(); i++){
        for(int j=0; j<pc_image[i].size(); j++)pc_image_copy[i][j] = pc_image[i][j];
    }
    for(int i=1; i<rings-1; i++){
        for(int j=1; j<pc_image.size()-1; j++){
            ...
            if(sum_n >= 5 && pc_image[j][i] == -1){
                pc_image[j][i] = sum_dis / sum_n;
                continue;
            }
            if(near_sum_n > sum_n / 2){
                pc_image[j][i] = near_sum_dis / near_sum_n;
            }
            if(far_sum_n > sum_n / 2){
                pc_image[j][i] = far_sum_dis / far_sum_n;
            }
        }
    }
    for(int i=0; i<rings; i++){
        if(i==0){
            for(int j=0; j<pc_image.size(); j++) {
                if(j==0){
                    if(pc_image[j][i+1] == -1 && pc_image[j+1][i] == -1)pc_image[j][i] = -1;
                    ...
                    if(pc_image[j][i+1] == -1 && pc_image[j-1][i] == -1 && pc_image[j+1][i] == -1)pc_image[j][i] = -1;
                    pc_image_copy[j][i] = pc_image[j][i];
                }
            }
        }
            ...
            for(int j=0; j<pc_image.size(); j++) {
                if(j==0){
                    if(pc_image[j][i-1] == -1 && pc_image[j+1][i] == -1 && pc_image[j][i+1] == -1)pc_image[j][i] = -1;
                    pc_image_copy[j][i] = pc_image[j][i];
                }
                else if(j==pc_image.size()-1){
                    if(pc_image[j][i-1] == -1 && pc_image[j-1][i] == -1 && pc_image[j][i+1] == -1)pc_image[j][i] = -1;
                    ...
                    if(pc_image[j][i+1] == -1 && pc_image[j][i-1] == -1 && pc_image[j-1][i] == -1 && pc_image[j+1][i] == -1)pc_image[j][i] = -1;
                    pc_image_copy[j][i] = pc_image[j][i];
                }
            }
        }
    ...
    for(int i=0; i<rings; i++){
        ...
        for(int j=0; j<pc_image.size(); j++){
            if(pc_image[j][i] != -1){
                mk.push_back(pc_image[j][i]);
                mk.push_back(j);
            }
        }
        if(mk.size()<6)continue;
        for(int j=1; j<(mk.size() / 2) - 1; j++){
            if(mk[(j-1)*2] - mk[(j)*2] > mk[(j)*2] / config["dis_threshold"].as<float>() ||
               mk[(j+1)*2] - mk[(j)*2] > mk[(j)*2] / config["dis_threshold"].as<float>() ||
               mk[j*2+1] - mk[(j-1)*2+1] > local_config["angle_pixel_dis"].as<int>() ||
               mk[(j+1)*2+1] - mk[j*2+1] > local_config["angle_pixel_dis"].as<int>() ||
               local_config["show_all"].as<bool>()){
                pcl::PointXYZI p;
                p.x = mk[(j)*2] * std::cos((i * factor_t + _lowerBound) * M_PI / 180) * std::cos((mk[j*2+1] * 0.18 - 90) * M_PI / 180);
                p.y = mk[(j)*2] * std::cos((i * factor_t + _lowerBound) * M_PI / 180) * std::sin(-(mk[j*2+1] * 0.18 - 90) * M_PI / 180);
                p.z = mk[(j)*2] * std::sin((i * factor_t + _lowerBound) * M_PI / 180);
                p.intensity = 0.5;
...
void extract_pc_feature_6(pcl::PointCloud<pcl::PointXYZI>::Ptr pc, pcl::PointCloud<pcl::PointXYZI>::Ptr pc_feature) {
    ...
    for(int i=1; i<rings-1; i++){
        for(int j=1; j<pc_image.size()-1; j++){
            ...
            if(pc_image_copy[j-1][i-1] != -1){
                if(pc_image[j][i] != -1){
                    if(pc_image_copy[j-1][i-1] - pc_image[j][i] > dis_threshold){//如果相邻点比此点远于一定阈值
                        far_sum_n ++;
                        far_sum_dis += pc_image_copy[j-1][i-1];
                    }
                    else if(pc_image[j][i] - pc_image_copy[j-1][i-1] > dis_threshold){//如果此点比相邻点远于一定阈值
                        near_sum_n ++;
                        near_sum_dis += pc_image_copy[j-1][i-1];
                    }
                }
    ...
    for(int i=0; i<rings; i++){
        if(i==0){//处理第一列
            for(int j=0; j<pc_image.size(); j++) {
                if(j==0){//处理pc_image第一行
                    if(pc_image[j][i+1] == -1 && pc_image[j+1][i] == -1)pc_image[j][i] = -1;//如果下一列和下一行都为-1，则为-1
                    pc_image_copy[j][i] = pc_image[j][i];//否则为pc_image
                }
                else if(j==pc_image.size()-1){//处理pc_image最后一行
                    if(pc_image[j][i+1] == -1 && pc_image[j-1][i] == -1)pc_image[j][i] = -1;//如果下一列和上一行都为-1，则为-1
                    pc_image_copy[j][i] = pc_image[j][i];
                }
                else{
                    if(pc_image[j][i+1] == -1 && pc_image[j-1][i] == -1 && pc_image[j+1][i] == -1)pc_image[j][i] = -1;//如果下一列和上一行和下一行都为-1，则为-1
                    pc_image_copy[j][i] = pc_image[j][i];
                }
            }
        }
        else if(i==rings-1){//处理最后一列
            for(int j=0; j<pc_image.size(); j++) {
                if(j==0){
                    if(pc_image[j][i-1] == -1 && pc_image[j+1][i] == -1)pc_image[j][i] = -1;//如果上一列和下一行都为-1，则为-1
                    pc_image_copy[j][i] = pc_image[j][i];
                }
                else if(j==pc_image.size()-1){
                    if(pc_image[j][i-1] == -1 && pc_image[j-1][i] == -1)pc_image[j][i] = -1;//如果上一列和上一行都为-1，则为-1
                    pc_image_copy[j][i] = pc_image[j][i];
                }
                else{
                    if(pc_image[j][i-1] == -1 && pc_image[j-1][i] == -1 && pc_image[j+1][i] == -1)pc_image[j][i] = -1;//如果上一列和上一行和下一行都为-1，则为-1
                    pc_image_copy[j][i] = pc_image[j][i];
                }
            }
        }
            ...
            for(int j=0; j<pc_image.size(); j++) {
                if(j==0){
                    if(pc_image[j][i-1] == -1 && pc_image[j+1][i] == -1 && pc_image[j][i+1] == -1)pc_image[j][i] = -1;//如果上一列和下一行和下一列都为-1.则为-1
                    pc_image_copy[j][i] = pc_image[j][i];
                }
                else if(j==pc_image.size()-1){
                    if(pc_image[j][i-1] == -1 && pc_image[j-1][i] == -1 && pc_image[j][i+1] == -1)pc_image[j][i] = -1;//如果上一列和上一行和下一列都为-1，则为-1
                    pc_image_copy[j][i] = pc_image[j][i];
                }
                else{
                    if(pc_image[j][i+1] == -1 && pc_image[j][i-1] == -1 && pc_image[j-1][i] == -1 && pc_image[j+1][i] == -1)pc_image[j][i] = -1;//如果下一列和上一列和上一行和下一行都为-1，则为-1
                    pc_image_copy[j][i] = pc_image[j][i];
                }
            }
        }
    ...
    for(int i=0; i<rings; i++){
        for(int j=1; j<pc_image.size()-1; j++) {
            if(pc_image[j][i] == -1){//如果此点为-1，但上一行和下一行都不为-1，取上一行或下一行较小的那个
                if(pc_image_copy[j-1][i] != -1 && pc_image_copy[j+1][i] != -1){
                    pc_image[j][i] = pc_image_copy[j-1][i] > pc_image_copy[j+1][i] ? pc_image_copy[j+1][i] : pc_image_copy[j-1][i];
                }
            }
    ...
    for(int j=0; j<pc_image.size(); j++){//1000
    ...
    {
        std::vector<float> mk;
        for(int j=0; j<pc_image.size(); j++){//j<1000
            if(pc_image[j][i] != -1)
            {//计算此列不等于-1的点
        ...
        #define DIS 0.1
        for(int j=1; j<(mk.size() / 2) - 1; j++){//mk的size等于一列(1000)个中不等于-1的个数×2
            // if(mk[(j-1)*2]!=-1 && (
            if(
                ...
                local_config["show_all"].as<bool>())
            {
                ...
                pcl::PointXYZI p;
                p.x = mk[(j)*2] * std::cos((i * factor_t + _lowerBound) * M_PI / 180.0) * std::cos((mk[j*2+1] * 0.18 - 90) * M_PI / 180.0);
                p.y = mk[(j)*2] * std::cos((i * factor_t + _lowerBound) * M_PI / 180.0) * std::sin(-(mk[j*2+1] * 0.18 - 90) * M_PI / 180.0);
                p.z = mk[(j)*2] * std::sin((i * factor_t + _lowerBound) * M_PI / 180.0);
    ...
    for(int i=0; i<pc_image.size(); i++)//i<1000
    {
        ...
        {//j=1;j<64-1;j++
                ...
                local_config["show_all"].as<bool>())
            {
                ...
                p.x = mk[(j)*2] * std::cos((j * factor_t + _lowerBound) * M_PI / 180) * std::cos((i * 0.18 - 90) * M_PI / 180);
                p.y = mk[(j)*2] * std::cos((j * factor_t + _lowerBound) * M_PI / 180) * std::sin(-(i * 0.18 - 90) * M_PI / 180);
                p.z = mk[(j)*2] * std::sin((mk[j*2+1] * factor_t + _lowerBound) * M_PI / 180);
    ...
    for (size_t i = 0; i < local_indices.size(); i++)
    {
		...
		if(obj_info.bounding_box_.dimensions.x > 0.5 || obj_info.bounding_box_.dimensions.y > 0.5 \
           || obj_info.bounding_box_.dimensions.z > 0.5)
		{
    ...
    for(size_t j = 0; j < obj_list.size(); ++j)
    {
        ...
        for(size_t i = 0; i < pc_feature->points.size(); ++i)
        {
            ...
            {
                pcl::PointXYZI p;
                p.x = pc_x; p.y = pc_y; p.z = pc_z; p.intensity = pc_feature->points[i].intensity;
...
void ExtractLineSegment(cv::Mat &img, cv::Mat &image2, std::vector<cv::line_descriptor::KeyLine> &keylines,std::vector<cv::line_descriptor::KeyLine> &keylines2,
cv::Mat &outimg, cv::Mat &outimg_thin)
{
    ...
    if(keylines.size()>lsdNFeatures){
        std::sort(keylines.begin(), keylines.end(),[](const cv::line_descriptor::KeyLine &a,const cv::line_descriptor::KeyLine &b){return a.response > b.response;});
        keylines.resize(lsdNFeatures);
        for( int i=0; i<lsdNFeatures; i++)
            keylines[i].class_id = i;
    }
    if(keylines2.size()>lsdNFeatures){
        std::sort(keylines2.begin(), keylines2.end(), [](const cv::line_descriptor::KeyLine &a,const cv::line_descriptor::KeyLine &b){return a.response > b.response;});
    ...
    std::string line_feature_string = "/home/zh/code/useful_tools/auto_calibration/data/result/line_feature_"+m_string+".png";
        ...
        for (int i = 0; i < img_draw.cols; i++) {
            for (int j = 0; j < img_draw.rows; j++) {
                if(i >= 5 && j >= 5 && i <= (img_draw.cols-5) && j <= (img_draw.rows-5)){
                    ...
                    // 竖直特征
                    if(cen==255 && (lu==255||up==255||ru==255||ld==255||dw==255||rd==255)
                    ...
                    // // 水平特征
                    if(cen==255 && (lu==255||lt==255||ld==255||rt==255||ru==255||rd==255||
 ...
 std::vector<cv::Mat>& distance_image, std::vector<cv::Mat>& distance_image_thin)
{
    ...
    float qian, hou, y_min, y_max, z_min, z_max;
    int search_num, search_num2, search_num3;
    qian = config["qian"].as<float>();
    hou = config["hou"].as<float>();
    ...
    for (int i = 0; i < data_num; i++) {
        ...
        pass_filter.setFilterLimitsNegative(false);
        pass_filter.setFilterLimits(hou, qian);
...
////
////        pass_filter.setFilterLimits(qian, FLT_MAX);
////        pass_filter.filter(*filtered);
////        r_filter.setInputCloud(filtered);
...
///*****************************Gazebo*****************************/
//                if (i > 7) {
//                    p.x = point_rings[i].points[0].x;
//                    p.y = point_rings[i].points[0].y;
//                    p.z = point_rings[i].points[0].z;
//                    p.intensity = 30.0;
//                    haha->push_back(p);
//                    p.x = point_rings[i].points[point_rings[i].size() - 1].x;
//                    p.y = point_rings[i].points[point_rings[i].size() - 1].y;
//                    p.z = point_rings[i].points[point_rings[i].size() - 1].z;
//                    p.intensity = 30.0;
//                    haha->push_back(p);
//                }
///***************************************************************/
...
//extract image edges
        ...
        std::string canny_string = "/home/zh/code/useful_tools/auto_calibration/data/result/canny"+i_string+".png";
...
#pragma omp parallel for
        ...
        cv::imwrite("/home/zh/code/useful_tools/auto_calibration/data/result/gray_image.png", gray_image);
        ...
        else if(config["line_features"].as<bool>())
        {
            ...
            ExtractLineSegment(image,image,keylines,keylines2,outimg,outimg_thin);
                 ...
                 Eigen::Matrix4f RT, Eigen::Matrix3f camera_param) 
{
    ...
    for (int j = 0; j < edge_size; j++) 
    {
        ...
        if (x < 0 || x > (distance_image.cols - 1) || y < 0 || y > (distance_image.rows - 1))continue;
        ...
        float pt_dis = pow(r.x*r.x+r.y*r.y+r.z*r.z, float(1/3));
...
float countConfidence(const pcl::PointCloud<pcl::PointXYZI>::Ptr pc_feature, const cv::Mat distance_image, 
                 Eigen::Matrix4f RT, Eigen::Matrix3f camera_param) 
{
    float score = 0;
    Eigen::Matrix<float, 3, 4> RT_TOP3, RT_X_CAM;//lida2image=T*(T_cam02cam2)*T_cam2image
...
{
    config = YAML::LoadFile("/home/zh/code/useful_tools/auto_calibration/config.yaml");
    ...
    T_imu2velo(0,0) = 9.999976e-01;     T_imu2velo(0,1) = 7.553071e-04;     T_imu2velo(0,2) = -2.035826e-03;    T_imu2velo(0,3) = -8.086759e-01;
    T_imu2velo(1,0) = -7.854027e-04;    T_imu2velo(1,1) = 9.998898e-01;     T_imu2velo(1,2) = -1.482298e-02;    T_imu2velo(1,3) = 3.195559e-01;
    T_imu2velo(2,0) = 2.024406e-03;     T_imu2velo(2,1) = 1.482454e-02;     T_imu2velo(2,2) = 9.998881e-01;     T_imu2velo(2,3) = -7.997231e-01;
    T_imu2velo(3,0) = 0;                T_imu2velo(3,1) = 0;                T_imu2velo(3,2) = 0;                T_imu2velo(3,3) = 1;
    
    R_imu2velo(0,0) = 9.999976e-01;     R_imu2velo(0,1) = 7.553071e-04;     R_imu2velo(0,2) = -2.035826e-03;
    R_imu2velo(1,0) = -7.854027e-04;    R_imu2velo(1,1) = 9.998898e-01;     R_imu2velo(1,2) = -1.482298e-02;
    R_imu2velo(2,0) = 2.024406e-03;     R_imu2velo(2,1) = 1.482454e-02;     R_imu2velo(2,2) = 9.998881e-01;
    ...
    if(config["save_pointcloud"].as<bool>())
    {
        ...
        std::string output_pc_name = "/home/zh/code/useful_tools/auto_calibration/data/pc";
    ...
    T_cam2image(0, 0) = 721.5377;
    T_cam2image(0, 1) = 0.0;
    T_cam2image(0, 2) = 609.5593;
    T_cam2image(1, 0) = 0.0;
    T_cam2image(1, 1) = 721.5377;
    T_cam2image(1, 2) = 172.854;
    T_cam2image(2, 0) = 0.0;
    T_cam2image(2, 1) = 0.0;
    T_cam2image(2, 2) = 1;
    ...
    cv::Mat outimg;
    std::vector<cv::line_descriptor::KeyLine> keylines,keylines2;
    // ExtractLineSegment(images[0],images[1],keylines,keylines2,outimg);
...
std::string calibrated_result_path = "/home/zh/code/useful_tools/auto_calibration/data/result/calibrated_result";
...
for(int ppp = 0; ppp<pcs.size(); ppp++){
    ...
    if(ppp>0)
    {
        ...
        std::cout<<"ICP -- transform is "<<std::endl<<transformation<<std::endl;
...
#pragma omp parallel for
        ...
        std::cout << "(new_rx, new_ry, new_rz) = " << bias_x + rot_x_adjust << ", " << bias_y + rot_y_adjust << 
        ", " << bias_z + rot_z_adjust << std::endl;
    ...
    if(config["many_or_one"].as<int>() == 3) 
    {
        ...
        if(ppp<pcs.size()-2 && config["3frame_score"].as<bool>())
        {
            ...
            {
                max_score = (countScore(pc_features_local[ppp], distance_images_local_thin[ppp], T_lidar2cam2_bias, T_cam2image)
                    +countScore(pc_features_local[ppp+1], distance_images_local_thin[ppp+1], T_lidar2cam2_bias, T_cam2image)
                    +countScore(pc_features_local[ppp+2], distance_images_local_thin[ppp+2], T_lidar2cam2_bias, T_cam2image)
                    )/3.0;
            }
            else
            {
                max_score = (countScore(pc_features_local[ppp], distance_images_local[ppp], T_lidar2cam2_bias, T_cam2image)
                    +countScore(pc_features_local[ppp+1], distance_images_local[ppp+1], T_lidar2cam2_bias, T_cam2image)
                    +countScore(pc_features_local[ppp+2], distance_images_local[ppp+2], T_lidar2cam2_bias, T_cam2image)
                    )/3.0;
            }
        }
        else
            ...
            for (int i = 0; i < 27; i++) {
                ...
                if(ppp<pcs.size()-2 && config["3frame_score"].as<bool>())
                {
                    ...
                    else
                    {
                        score = (countScore(pc_features_local[ppp], distance_images_local[ppp], T_lidar2cam2_bias_copy, T_cam2image)
                            ...
                            +countScore(pc_features_local[ppp+2], distance_images_local[ppp+2], T_lidar2cam2_bias_copy, T_cam2image)
                            )/3.0;
                    }
                }
                else
                ...
                omp_set_lock(&mylock);
                {
                    if (score > max_score) {
                        ...
                        // i = 0;
                        max_score = score;
                        T_lidar2cam2_bias(0,0) = T_lidar2cam2_bias_copy(0, 0);T_lidar2cam2_bias(0,1) = T_lidar2cam2_bias_copy(0, 1);
                        T_lidar2cam2_bias(0,2) = T_lidar2cam2_bias_copy(0, 2);T_lidar2cam2_bias(0,3) = T_lidar2cam2_bias_copy(0, 3);
                        T_lidar2cam2_bias(1,0) = T_lidar2cam2_bias_copy(1, 0);T_lidar2cam2_bias(1,1) = T_lidar2cam2_bias_copy(1, 1);
                        T_lidar2cam2_bias(1,2) = T_lidar2cam2_bias_copy(1, 2);T_lidar2cam2_bias(1,3) = T_lidar2cam2_bias_copy(1, 3);
                        T_lidar2cam2_bias(2,0) = T_lidar2cam2_bias_copy(2, 0);T_lidar2cam2_bias(2,1) = T_lidar2cam2_bias_copy(2, 1);
                        T_lidar2cam2_bias(2,2) = T_lidar2cam2_bias_copy(2, 2);T_lidar2cam2_bias(2,3) = T_lidar2cam2_bias_copy(2, 3);
                        T_lidar2cam2_bias(3,0) = T_lidar2cam2_bias_copy(3, 0);T_lidar2cam2_bias(3,1) = T_lidar2cam2_bias_copy(3, 1);
                        T_lidar2cam2_bias(3,2) = T_lidar2cam2_bias_copy(3, 2);T_lidar2cam2_bias(3,3) = T_lidar2cam2_bias_copy(3, 3);
                        better_cnt++;
                    }
                }
                omp_unset_lock(&mylock);
...
*/
        ...
        {
            Eigen::Matrix3f R_lidar2cam2_bias_delt1 = T_lidar2cam2_bias_vec[T_lidar2cam2_bias_vec.size()-2].block(0,0,3,3).inverse()
                                                     * T_lidar2cam2_bias_vec[T_lidar2cam2_bias_vec.size()-1].block(0,0,3,3);
            Eigen::Matrix3f R_lidar2cam2_bias_delt2 = T_lidar2cam2_bias_vec[T_lidar2cam2_bias_vec.size()-3].block(0,0,3,3).inverse()
                                                     * T_lidar2cam2_bias_vec[T_lidar2cam2_bias_vec.size()-1].block(0,0,3,3);
            ...
            std::cout << "R euler delt = " << abs(R_euler_xyz_delt1[0]-R_euler_xyz_delt2[0]) * 180.0 / M_PI  << " " << 
            abs(R_euler_xyz_delt1[1]-R_euler_xyz_delt2[1]) * 180.0 / M_PI  << " " << abs(R_euler_xyz_delt1[2]-R_euler_xyz_delt2[2]) * 180.0 / M_PI  << std::endl;
    ...
    std::cout<<"euler angle calibrated = " << std::endl << euler_ang_calibrated_zyx * 180.0 / M_PI << std::endl
    << "euler angle gt = " << std::endl << euler_ang_gt_zyx * 180.0 / M_PI << std::endl;

    std::cout<<"euler angle delt = " << std::endl << euler_ang_delt_xyz[0] * 180.0 / M_PI << " " << euler_ang_delt_xyz[1] * 180.0 / M_PI 
    << " " << euler_ang_delt_xyz[2] * 180.0 / M_PI << std::endl;

    #ifdef USE_ORB
    if(ppp > 0)
    {
        std::cout<<"R_cam_Del = " << std::endl << R_cam_del << std::endl;
        ...
        std::cout << "T_lidar2cam2_unbias = " << std::endl << T_lidar2cam2_unbias.block(0,0,3,3) << std::endl;
        std::cout << "T_lidar2cam2_bias = " << std::endl << T_lidar2cam2_bias.block(0,0,3,3) << std::endl;
        std::cout << "R_lidar2cam2_cal = " << std::endl << R_lidar2cam2_cal << std::endl;
    ...
    cv::Mat dst;
    dst.create(image1.rows * 3, image1.cols, CV_8UC3);
    ...
    // 读取文本数据
    std::ifstream inFile("/home/zh/code/useful_tools/auto_calibration/data/pc_lines");
    ...
    if (inFile) // 有该文件  
    {
        ...
        while (getline(inFile, lineStr)) // line中不包括每行的换行符  
        {
            ...
            pc_temp.x = data_temp[0]; pc_temp.y = data_temp[1]; pc_temp.z = data_temp[2];
    ...
    cv::imwrite("/home/zh/code/useful_tools/auto_calibration/data/result/project_result_"+std::to_string(ppp)+".png", image_result);
    cv::imwrite("/home/zh/code/useful_tools/auto_calibration/data/result/original_bias_"+std::to_string(ppp)+".png", image2);
    cv::imwrite("/home/zh/code/useful_tools/auto_calibration/data/result/unbias_"+std::to_string(ppp)+".png", image_unbias);
    cv::imwrite("/home/zh/code/useful_tools/auto_calibration/data/result/calibrated_result_"+std::to_string(ppp)+".png", image4);
...
//    cv::imwrite("/home/mkjhnb/gazebo_ws/test/pic_3" + std::to_string(ppp) + ".png", dst);
    ...
    if(config["save_calibrated_result"].as<bool>())
    {
        ...
        if(calibrated_result)
        {
            ...
            calibrated_result << "]" << std::endl << std::endl;
