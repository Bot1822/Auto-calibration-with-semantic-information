...
private:
    ...
    //float score1_infer_cloud = 0;  //ȫ�ֱ�����ôд����ʵû�ĵ�
    //float score2_filter_cloud = 0;  

    void evaluation_function(pcl::PointCloud<pcl::PointXYZI>::Ptr &infer_cloud, pcl::PointCloud<pcl::PointXYZI>::Ptr &filter_cloud, pcl::PointCloud<pcl::PointXYZI>::Ptr &truth_cloud,std::vector <float> &score_count_cloud);
    void Get_other_images(std::string txtName, std::string folderName, std::vector<cv::Mat> &other_images) ;
    void Get_message_filter_cloud(std::string txtName, std::string folderName, std::vector<pcl::PointCloud<pcl::PointXYZI>::Ptr> &message_filter_cloud);
    void Get_truth_cloud(std::string txtName, std::string folderName, std::vector<pcl::PointCloud<pcl::PointXYZI>::Ptr> &message_filter_cloud);
    ...
    void icp_erase(pcl::PointCloud<pcl::PointXYZI>::Ptr &pc1,
               pcl::PointCloud<pcl::PointXYZI>::Ptr &pc2, Eigen::Matrix4f &T_v,pcl::PointCloud<pcl::PointXYZI>::Ptr &dynamic_cloud);
